  ...
  return (
    <Container className="py-10">
      <div className="grid lg:grid-cols-2 gap-8">
          ...
          {selectedOrderType === 'delivery' && (
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-600">Choose your delivery speed</p>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-3 gap-3">
                  <Button
                    variant={selectedDeliveryService === 'standard' ? 'default' : 'outline'}
                    onClick={() => setSelectedDeliveryService('standard')}
                    className="h-12 flex flex-col items-center justify-center"
                  >
                    <div className="text-sm font-medium">Standard</div>
                    <div className="text-xs text-gray-500">Regular delivery</div>
                  </Button>
                  
                  <Button
                    variant={selectedDeliveryService === 'premium' ? 'default' : 'outline'}
                    onClick={() => setSelectedDeliveryService('premium')}
                    className="h-12 flex flex-col items-center justify-center"
                  >
                  ...
                  <Button
                    ...
                    onClick={() => setSelectedDeliveryService('priority')}
                    className="h-12 flex flex-col items-center justify-center"
                  >
        ...
        <div className="space-y-6">
          {editorMode && fulfillableStores.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Edit Items by Store</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {fulfillableStores.map((store: any) => {
                    ...
                    return (
                    <div key={store.store_id} className={`border rounded-lg p-3 space-y-2 ${isMarked ? 'opacity-50 pointer-events-none' : ''}`}>
                      <div className="flex items-center justify-between">
                        <div className="font-medium text-sm flex items-center gap-2">
                          {store.store_name}
                          {isMarked && (<span className="text-[10px] px-2 py-0.5 rounded bg-gray-200 text-gray-700">Removed</span>)}
                        </div>
                        <Button variant="destructive" size="sm" onClick={() => handlePromptDeleteStore(store.store_id)} disabled={isMarked}>Delete All</Button>
                      </div>
                      ...
                      <div className="max-h-56 overflow-y-auto space-y-2">
                        {(store.items || []).map((it: any, idx: number) => {
                          ...
                          return (
                            <div key={`${store.store_id}-${it.product_id ?? it.id ?? idx}`} className="flex items-center gap-3 text-sm">
                              ...
                              <div className="text-sm font-medium">LKR {unitPrice.toFixed(2)}</div>
                            </div>
                          );
                        })}
                      </div>
                ...
                <div className="mt-4 flex justify-end">
                  <Button onClick={handleSaveEditor} disabled={loadingPreview}>Save</Button>
                </div>
              </CardContent>
            </Card>
          )}
      ...
      <Dialog open={showMultiStoreDialog} onOpenChange={setShowMultiStoreDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Multiple stores will fulfill this order</DialogTitle>
            <DialogDescription>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {fulfillableStores.map((s: any, sIdx: number) => (
              <div key={s.store_id ?? `store-${sIdx}`} className="border rounded-lg p-3 text-sm space-y-1">
          ...
          <div className="mt-2 text-right font-semibold">Overall total: LKR {(backendData?.overall_total || 0).toFixed(2)}</div>
          <DialogFooter>
            <Button variant="outline" onClick={handleConfirmSplitNo}>No, I'll adjust</Button>
            <Button onClick={handleConfirmSplitYes}>Yes, place {fulfillableStores.length} orders</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={confirmStoreId != null} onOpenChange={(open) => { if (!open) setConfirmStoreId(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete all items from this store?</DialogTitle>
            <DialogDescription>
              This will remove all items from the selected store from your cart.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={handleCancelDeleteStore}>Cancel</Button>
            <Button variant="destructive" onClick={handleConfirmDeleteStore}>Yes, delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Container>
