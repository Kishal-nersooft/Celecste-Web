    ...
    <Card>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-red-800">Some items unavailable</p>
            ...
            <div className="max-h-48 overflow-y-auto space-y-2">
              {unavailable_items.map((ui: any, idx: number) => (
                <div key={`unavail-${ui.product_id ?? idx}`} className="flex items-center gap-3 text-sm">
                  <div className="w-10 h-10 bg-white rounded border flex items-center justify-center overflow-hidden">
                    {ui.image_url ? (
                      <Image src={ui.image_url} alt={ui.name || `#${ui.product_id}`} width={40} height={40} className="object-cover w-full h-full" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500">Requested: {ui.requested_quantity || ui.quantity || 0}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3">
          {isMultiStore ? (
            <div className="space-y-2">
              <h4 className="font-medium text-gray-900">Items by store</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {fulfillable_stores.map((store: any, sIdx: number) => (
                  <div key={store.store_id ?? `store-${sIdx}`} className="border rounded-lg p-3 space-y-2">
                    <div className="font-medium text-sm">{store.store_name}</div>
                    <div className="text-xs text-gray-500">{(store.items?.length || 0)} items • Subtotal LKR {(store.subtotal || 0).toFixed(2)}</div>
                    <div className="max-h-56 overflow-y-auto space-y-2">
                      {(store.items || []).map((it: any, idx: number) => {
                        ...
                        return (
                          <div key={`${store.store_id ?? `store-${sIdx}`}-${it.product_id ?? it.id ?? idx}`} className="flex items-center gap-3 text-sm">
                            <div className="w-10 h-10 bg-gray-100 rounded flex items-center justify-center overflow-hidden">
                              {hasValidImage ? (
                                <Image src={imageUrl as string} alt={productName} width={40} height={40} className="object-cover w-full h-full" />
                              ) : (
                                <ShoppingCart className="h-4 w-4 text-gray-400" />
                              )}
                            </div>
                            ...
                            <div className="text-sm font-medium">LKR {lineTotal.toFixed(2)}</div>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="space-y-3">
                  <div className="max-h-60 overflow-y-auto space-y-2">
                    {backendItems.map((backendItem: any, index: number) => {
                      ...
                      return (
                        <div key={`${backendItem.product_id ?? index}`} className="flex items-center gap-3 p-3 border rounded-lg">
                          <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                            {hasValidImage ? (
                              <Image src={imageUrl} alt={productName} width={48} height={48} className="w-full h-full object-cover" />
                            ) : (
                              <ShoppingCart className="h-6 w-6 text-gray-400" />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h5 className="font-medium text-sm truncate">{productName}</h5>
                            <p className="text-[11px] text-gray-500">ID: {backendItem.product_id}</p>
                            <p className="text-xs text-gray-500">Qty: {backendItem.quantity} • Unit: LKR {finalPrice.toFixed(2)}</p>
                            <div className="flex items-center gap-2">
                              {isDiscounted ? (
                                <>
                                  <span className="text-sm font-bold text-red-600">LKR {totalPrice.toFixed(2)}</span>
                                  <span className="text-xs text-gray-400 line-through">LKR {totalBasePrice.toFixed(2)}</span>
                                </>
                              ) : (
                                <span className="text-sm font-medium text-gray-900">LKR {totalPrice.toFixed(2)}</span>
                              )}
                            </div>
                          </div>
                        </div>
        ...
        {unavailable_items && unavailable_items.length === 0 && backendItems.length > 0 && (
          <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-green-800">All items available</p>
        ...
        <button
          onClick={onCheckout}
          disabled={loadingCheckout || cartItems.length === 0}
          className="w-full bg-black text-white py-3 px-6 rounded-lg font-bold hover:bg-gray-800 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
        >
          {loadingCheckout ? (
            <div className="flex items-center justify-center gap-2">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
