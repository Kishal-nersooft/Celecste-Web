...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  // Check if product has pricing data and is discounted (discount_applied > 0)
  ...
  } else if (product.pricing && product.pricing.discount_applied > 0 && !isDiscounted) {
    console.error(`❌ Discount detection failed for product ${product.id}:`, product.pricing);
  }
  
  const discountInfo = isDiscounted ? {
  ...
  return (
    <div className={`border border-gray-300 rounded-lg overflow-hidden group text-xs flex flex-col h-[240px] w-full max-w-[180px] relative ${
      isDiscounted ? 'bg-black text-white' : 'bg-gray-100'
    }`}>
      <div className={`border-b overflow-hidden relative h-1/2 ${
        ...
        {isDiscounted && discountInfo.percentage > 0 && (
          <div className="absolute top-2 left-2 z-10">
            <div className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md">
              ...
              {discountInfo.isKnown && <span className="text-xs opacity-75">*</span>}
            </div>
          </div>
        )}
      ...
      <div className="p-2 flex flex-col gap-1 h-1/2 justify-between pr-8">
        <PriceView
          price={product?.pricing?.final_price || product?.base_price || product?.price}
          originalPrice={isDiscounted ? product?.pricing?.base_price : undefined}
          isDiscounted={isDiscounted}
        />
        <p className={`text-[10px] font-bold tracking-wide line-clamp-1 uppercase ${
          isDiscounted ? 'text-white' : 'text-black'
        }`}>
          {product?.brand}
        </p>
        <p className={`text-sm tracking-wide font-semibold line-clamp-1 capitalize ${
          isDiscounted ? 'text-white' : 'text-black'
        }`}>
          {product?.name}
        </p>
        <p className={`text-[10px] tracking-wide line-clamp-1 ${
